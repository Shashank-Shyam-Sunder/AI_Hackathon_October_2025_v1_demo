# src/app/llm/providers/google.py
...
class GoogleSelector:
    ...
    def __init__(self, cfg: LLMConfig):
        ...
        try:
            ...
            raise RuntimeError(
                "Missing Google Gemini LangChain integration. Install with "
                "'pip install langchain langchain-google-genai'") from e

        api_key = os.getenv("GOOGLE_API_KEY") or os.getenv("GOOGLE_GENAI_API_KEY")
        if not api_key:
            raise RuntimeError("Google API key not found. Set GOOGLE_API_KEY or GOOGLE_GENAI_API_KEY.")

        # Build client (Gemini ignores some decoding params; we keep temp=0 for consistency)
    ...
    def select(self, candidates: List[Dict[str, Any]], intake: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            raise RuntimeError(f"Google Gemini call failed: {e}") from e
